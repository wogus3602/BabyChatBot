...
public class ChatsAdapter extends RecyclerView.Adapter<ChatsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (message.getMessageType() == MessageType.QuickReplies) {
            LinearLayoutManager lm = new LinearLayoutManager(context);
            lm.setOrientation(LinearLayoutManager.HORIZONTAL);
            holder.recyclerView.setLayoutManager(lm);
            holder.recyclerView.setAdapter(new QuickRepliesAdapter(context, message.getQuicks()));
            return;
        }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        }else if (viewType == MessageType.QuickReplies.ordinal()) {  //quick re
            v = LayoutInflater.from(context).inflate(R.layout.chat_user1_quickreplies_item, parent, false);
            return new ViewHolder(v, MessageType.QuickReplies);
        }
        return null;
    }
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ViewHolder(View v, MessageType messageType) {
            ...
            if (messageType == MessageType.QuickReplies) {
                recyclerView = v.findViewById(R.id.quickreplies);
            }
